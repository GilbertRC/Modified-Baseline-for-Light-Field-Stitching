...
def make_epiinput_eslf(image_path,seq1,image_h,image_w,view_n,RGB):
    traindata_tmp=np.zeros((1,image_h,image_w,len(view_n)),dtype=np.float32)
    i=0
    if(len(image_path)==1):
        image_path=image_path[0]
    ...
    for seq in seq1:
        #tmp  = np.float32(imageio.imread(image_path+'/input_Cam0%.2d.png' % seq)) 
        tmp  = np.float32(imageio.imread(image_path+'/input_Cam%d.png' % seq)) 
        traindata_tmp[0,:,:,i]=(RGB[0]*tmp[:,:,0] + RGB[1]*tmp[:,:,1] + RGB[2]*tmp[:,:,2])/255
        i+=1
    return traindata_tmp
...
def make_multiinput(image_path,image_h,image_w,view_n):
    ...
    elif(image_path[:4]=='eslf'):
        val_90d=make_epiinput_eslf(image_path,seq90d,image_h,image_w,view_n,RGB)    
        val_0d=make_epiinput_eslf(image_path,seq0d,image_h,image_w,view_n,RGB)
        val_45d=make_epiinput_eslf(image_path,seq45d,image_h,image_w,view_n,RGB)
        val_M45d=make_epiinput_eslf(image_path,seqM45d,image_h,image_w,view_n,RGB) 
